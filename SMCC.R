...
"eig1" <- function( A, c = NA, isMax = NA, isSym = NA ) {
  ...
  eigval = d[idx1]
  eigvec = Re(v[,idx1])
  eigval_full = d[idx] # all eigen values
...
"fusion" <- function( X, c, no.dim = NA, k = 10 ) {
  ...
  for(iter in 1:NITER) {
    ...
    flflt = matrix(0,dim(D_Kernels[[1]])[1],dim(D_Kernels[[1]])[1])
    for(i in 1:length(D_Kernels)){
      fl =eig1(D_Kernels[[i]],c,0)$eigvec
      ttt = fl%*%t(fl)
      flflt = flflt + 0.1*ttt
    }
    eig1_res = eig1(L-flflt,c,0)
    
    F_eig1 = eig1_res$eigvec
    temp_eig1 = eig1_res$eigval
    ev_eig1 = eig1_res$eigval_full # all eig values
    evs_eig1 = cbind(evs_eig1,ev_eig1)# all eig values in every iter.
    DD = vector()
    # W * S
    for (i in 1:length(D_Kernels)) {
...
"SMCC" <- function(X,c,k=20,label_path=0,label_col=0) {
  test = fusion(X,c = c,k=k)
  test_S = test$S
  test_S = as.matrix(test_S)
  
  
  if(label_path==0|label_col==0){
    biaoqian = Slabel(test_S,c)
    rr = list()
    rr[["S"]] = test$S
    rr[['label']] = biaoqian
  }
  else{
    pingfen = Sscore(test_S,c,label_path,label_col)
    names(pingfen) = c('RI','ARI','NMI','Accu','F1')
    biaoqian = Slabel(test_S,c)
